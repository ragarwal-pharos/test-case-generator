...
class TestGeneratorService {
     ...
     */
    async generateTestsForFile(fileUri, progress, token) {
        const config = vscode.workspace.getConfiguration('testgen');
        const cliPath = config.get('cliPath', 'testgen');
        const outputDir = config.get('outputDirectory', 'tests');
        const framework = config.get('testFramework', 'xunit');
     ...
     */
    async generateTestsForProject(projectUri, progress, token) {
        const config = vscode.workspace.getConfiguration('testgen');
        const cliPath = config.get('cliPath', 'testgen');
        const outputDir = config.get('outputDirectory', 'tests');
        const framework = config.get('testFramework', 'xunit');
     ...
     */
    async getTestSuggestions(document, position) {
        // This would integrate with your AI service to get intelligent test suggestions
        const config = vscode.workspace.getConfiguration('testgen');
        const enableAI = config.get('enableAI', true);
        if (!enableAI) {
            return [];
        }
        try {
     ...
     */
    async executeTestGen(cliPath, args, progress, token) {
        return new Promise((resolve, reject) => {
            this.outputChannel.appendLine(`Executing: ${cliPath} ${args.join(' ')}`);
            const process = (0, child_process_1.spawn)(cliPath, args, {
     ...
     */
    parseProgressFromOutput(output, progress) {
        if (!progress)
            return;
    ...
    async checkCliAvailability() {
        ...
        return new Promise((resolve) => {
            const process = (0, child_process_1.spawn)(cliPath, ['--version'], { stdio: 'pipe' });
            process.on('close', (code) => {
                resolve(code === 0);
            });
            process.on('error', () => {
